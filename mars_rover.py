...
def F_drive():
    ...
    F_rrs = C_rr*get_mass()*planet['g']*math.cos(alpha)
    F_rr = math.erf(40*v)*(F_rrs)

    F_d = F_rr + get_mass()*planet['g']*math.sin(alpha)

    return F_rrs, F_rr, F_d, alpha
def F_gravity():

    alpha = int(input("what is the angle of the inclined terrain? "))
    F_xg = get_mass()*planet['g']*math.cos(alpha)
    F_yg = get_mass()*planet['g']*math.sin(alpha)
